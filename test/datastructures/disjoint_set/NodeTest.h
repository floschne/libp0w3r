...
#include "../../../src/datastructures/disjoint_set/DisjointSet.h"

using namespace libp0w3r;

TEST(disjointSetTests, NodeTestStandardCtor) {

    auto n1Ptr = std::make_shared<DisjointSet<int>::Node>();
...
TEST(disjointSetTests, NodeTestTwoParameterCtor) {
    ...
    int n2_data{1337};
    auto n2Ptr = std::make_shared<DisjointSet<int>::Node>(n2_data, n1Ptr);
...
TEST(disjointSetTests, NodeTestOneParameterCtor) {
    ...
    int n2_data{1337};
    auto n2Ptr = std::make_shared<DisjointSet<int>::Node>(n2_data);
...
TEST(disjointSetTests, NodeTestOneParameterMoveCtor) {
    ...
    int n2_data{1337};
    auto n2Ptr = std::make_shared<DisjointSet<int>::Node>(std::move(n2_data));
...
TEST(disjointSetTests, NodeTestTwoParameterMoveCtor) {
    ...
    int n2_data{1337};
    auto n2Ptr = std::make_shared<DisjointSet<int>::Node>(std::move(n2_data), std::move(n1Ptr));
...
TEST(disjointSetTests, NodeTestOutputOperator) {
    ...
    int n1_data{1337};
    DisjointSet<int>::Node n1(n1_data);
...
TEST(disjointSetTests, NodeTestSetters) {
    ...
    auto n2Ptr = std::make_shared<DisjointSet<int>::Node>(std::move(n1_data));
    n1.setParent(n2Ptr);
